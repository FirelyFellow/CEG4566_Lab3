...
int main(void)
{
  ...
  char code[] = "1234"; // This is what the user has to enter to arm or disarm the system.
  ...
  while (1)
  {
	...
	/* D10 to D7 as input pins for row 0 to row 3. D6 to D3 as output for column pins C1 to C3*/
	  ...
	  /*if (strcmp(code, userCode) == 0) {
		  SSD1306_Puts ("EQUAL", &Font_11x18, 1);
		  SSD1306_UpdateScreen();
	  }*/

	  if (key == '*')
	  {
		  if (strncmp(code, userCode, 4) == 0)
		  {
			  if (armed)
			  {
				  SSD1306_Puts ("Alarm OFF", &Font_11x18, 1);
				  armed = 0;
